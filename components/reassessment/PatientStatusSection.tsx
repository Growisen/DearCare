        ...
        {fields.map((field) => (
          <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            ...
            <div className="md:col-span-9">
              <textarea id={field.id} name={field.id} rows={1} value={formData[field.id as keyof FormData] as string} onChange={onChange} className="w-full border-b border-gray-200 px-0 py-1 focus:outline-none focus:border-gray-400 transition-colors resize-none bg-transparent placeholder-gray-300 text-gray-700 text-sm" placeholder="Enter details..." />
            </div>
          </div>
        ))}
        <div className="mt-4">
