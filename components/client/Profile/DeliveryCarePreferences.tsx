...
import { Calendar, Truck, Users, AlertCircle, Link as LinkIcon, ExternalLink } from "lucide-react";
...
const DeliveryCarePreferences: React.FC<DeliveryCarePreferencesProps> = ({ clientId }) => {
  ...
  if (isLoading) return (
    <div className="text-center text-gray-500 animate-pulse min-h-[400px]">
  ...
  if (error) return (
    <div className="text-center text-red-600 bg-red-50 rounded-sm border border-red-100 min-h-[400px]">
      <div className="flex justify-end space-x-2">
        {CopyLinkButton}
        {ExternalLinkButton}
      </div>
  ...
  if (!selectedRequest) return (
    <div className="text-center text-gray-500 italic min-h-[400px]">
      ...
      <div className="mt-4">No delivery care preferences found.</div>
    </div>
  );
