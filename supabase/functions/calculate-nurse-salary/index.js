...
Deno.serve(async () => {
  ...
  try {
    ...
    // Check if we should run the calculation
    ...
    for (const record of attendanceRecords) {
      ...
      // Check for missing data in the record - skip if any required data is missing
      ...
      if (missingDataItems.length > 0) {
        skippedMissingData++
        console.log(`Skipping record ${record.id} due to missing data: ${missingDataItems.join(', ')}`)
        
        // Track skipped records for this nurse
      ...
      if (workedHours <= 0) {
        skippedNoHours++
        console.log(`Skipping record ${record.id}: no valid hours - ${record.total_hours}`)
    ...
    const salaryRows = Array.from(nurseDataMap.values()).map(nurse => {
      // Create skipped records summary
      ...
      // Update info to include skipped records information
      ...
      if (nurse.skipped_records.length > 0) {
        infoText += ` | SKIPPED: ${nurse.skipped_records.length} records (${nurse.skipped_records.filter(r => r.reason === 'missing_data').length} missing data, ${nurse.skipped_records.filter(r => r.reason === 'invalid_hours').length} invalid hours)`
      }
