import { getEmailTransporter } from '@/config/nodeMailer.config'

function getBrandInfo(clientCategory: string) {
  if (clientCategory === 'DearCare LLP') {
    return {
      brandName: 'DearCare Health Services',
      supportEmail: 'support@dearcare.com',
      supportPhone: '(555) 123-4567',
      appName: 'DearCare',
    };
  } else {
    return {
      brandName: 'Tata Home Nursing',
      supportEmail: 'support@tatahomenursing.com',
      supportPhone: '(555) 987-6543',
      appName: 'Tata Home Nursing',
    };
  }
}
...
export async function sendClientCredentials(email: string, data: { 
  ...
  if (data.clientCategory === 'DearCare LLP') {
    user = process.env.DEARCARE_EMAIL_USER || '';
    pass = process.env.DEARCARE_EMAIL_PASSWORD || '';
  } else {
    user = process.env.TATAHOMENURSING_EMAIL_USER || '';
    pass = process.env.TATAHOMENURSING_EMAIL_PASSWORD || '';
  }

  const transporter = getEmailTransporter(user, pass);
  const currentYear = new Date().getFullYear();
  const brand = getBrandInfo(data.clientCategory);

  try {
    const info = await transporter.sendMail({
      from: `"${brand.brandName}" <${user}>`,
      to: email,
      subject: `${brand.appName} Account Activation - Confidential Information`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          ...
          <title>${brand.appName} Account Information</title>
        </head>
          ...
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
            ...
            <tr>
              <td style="background-color: #0056b3; padding: 20px; text-align: center;">
                <h1 style="color: white; margin: 0; font-weight: 600;">${brand.brandName}</h1>
              </td>
            </tr>
            <!-- Main Content -->
            <tr>
              <td style="padding: 30px 25px;">
                ...
                <p style="font-size: 16px; line-height: 1.5;">We are pleased to inform you that your application for ${brand.appName} services has been reviewed and approved by our healthcare team. Your account has been created, and you may now access our secure mobile application.</p>
                ...
                <ol style="font-size: 16px; line-height: 1.5;">
                  <li>Download the ${brand.appName} mobile application using <a href="${data.appDownloadLink}" style="color: #0056b3; text-decoration: underline;">this secure link</a></li>
                ...
                <p style="font-size: 16px; line-height: 1.5;">If you have any questions or require assistance with accessing your account, please contact our Support Team at <a href="mailto:${brand.supportEmail}" style="color: #0056b3; text-decoration: underline;">${brand.supportEmail}</a> or call us at ${brand.supportPhone}.</p>
                <p style="font-size: 16px; line-height: 1.5;">Thank you for choosing ${brand.appName} for your healthcare needs.</p>
                <p style="font-size: 16px; line-height: 1.5;">Sincerely,</p>
                <p style="font-size: 16px; line-height: 1.5; margin-bottom: 0;"><strong>The ${brand.appName} Team</strong></p>
              </td>
            </tr>
            <!-- Footer -->
            <tr>
              <td style="background-color: #f2f2f2; padding: 20px 25px; border-top: 1px solid #dddddd;">
                ...
                <p style="font-size: 12px; color: #666666; margin-bottom: 0;">&copy; ${currentYear} ${brand.brandName}. All rights reserved.</p>
              </td>
            </tr>
          </table>
        </body>
...
export async function sendClientFormLink(email: string, data: { 
  ...
  try {
    const info = await transporter.sendMail({
      from: `"${brand.brandName}" <${user}>`,
      to: email,
      subject: `${brand.appName} Health Assessment Form - Action Required`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>${brand.appName} Health Assessment Form</title>
        </head>
        <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333333; background-color: #f7f7f7;">
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
            <!-- Header -->
            ...
            <tr>
              <td style="padding: 30px 25px;">
                <p style="margin-top: 0; font-size: 16px; line-height: 1.5;">Dear ${data.name},</p>
                <p style="font-size: 16px; line-height: 1.5;">Thank you for choosing ${brand.appName} for your healthcare needs. To provide you with the best possible care, we need some additional information about your health status.</p>
                <div style="background-color: #f8f9fa; border-left: 4px solid #0056b3; padding: 15px; margin: 25px 0;">
...
export async function sendClientRejectionNotification(email: string, data: { 
  ...
  try {
    const info = await transporter.sendMail({
      from: `"${brand.brandName}" <${user}>`,
      to: email,
      subject: `${brand.appName} Application Status Update`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>${brand.appName} Application Status</title>
        </head>
        <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333333; background-color: #f7f7f7;">
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
            <!-- Header -->
            ...
            <tr>
              <td style="padding: 30px 25px;">
                <p style="margin-top: 0; font-size: 16px; line-height: 1.5;">Dear ${data.name},</p>
                <p style="font-size: 16px; line-height: 1.5;">Thank you for your interest in ${brand.brandName}. We have carefully reviewed your application for our services.</p>
                ...
                <p style="font-size: 16px; line-height: 1.5;">If you believe there has been an error or if you would like to discuss this decision further, please contact our Support Team at <a href="mailto:${brand.supportEmail}" style="color: #0056b3; text-decoration: underline;">${brand.supportEmail}</a> or call us at ${brand.supportPhone}.</p>
